...
async def _get_proxy():
    try:
        ...
        # logger.warning(f"winreg.OpenKey error {e}")
        return None
    try:
    ...
    if enabled == 1:
        ...
        try:
            ...
            config_file = await AsyncHTMLSession().get(config_file, timeout=10)
...
async def get_proxy():
    proxy1 = await sync_to_async(urllib.request.getproxies)()
    if proxy1 and "http" in proxy1:
        proxy = proxy1["http"]
    else:
    ...
    if proxy:
        ...
        os.environ.update(http_proxy="", https_proxy="")
    return proxy
